...
def eval_track_conductor(pred_pcd, gt_pcd, gt_flys, rgbs, tracker, commit=""):   # (b, f, h, w, 3) * 2, (b, f, h, w)

    b, f, h, w, _ = pred_pcd.shape

    # Align based on the first frame
    ...
    tracks, visibility = tracker(rgbs*255, grid_size=35)
    rgbs = rgbs[:, :f]
    tracks = tracks[:, :f]
    ...
    visibility = ((visibility * mask_track) == 1.0).float()
    ...
    if (visibility[:, -1].sum() <= 0.0) or (visibility.shape[-1] == 0):
        track_fde = None
    else:
        track_fde = (dis[:, -1] * visibility[:, -1]).sum() / visibility[:, -1].sum()
        track_fde = 1000.0 * track_fde.item()

    if (visibility.sum() <= 0.0) or (visibility.shape[-1] == 0):
        track_ade = None
