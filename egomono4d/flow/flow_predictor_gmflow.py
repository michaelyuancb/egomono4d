...
try:
    from ..repo.gmflow.gmflow.gmflow import GMFlow
except ImportError:
    GMFlow = None
...
class FlowPredictorGMFlow(FlowPredictor[FlowPredictorGMFlowCfg]):
    def __init__(self, cfg: FlowPredictorGMFlowCfg) -> None:
        ...
        checkpoint_path = cfg.cache_dir + "/gmflow_checkpoints/" + checkpoint
    ...
    def forward(
        ...
        wh = torch.tensor((w-1, h-1), dtype=torch.float32, device=flow.device)
