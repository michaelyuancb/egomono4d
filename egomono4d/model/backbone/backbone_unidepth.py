...
class BackboneUniDepth(Backbone[BackboneUniDepthCfg]):
    def __init__(
        self,
        cfg: BackboneUniDepthCfg,
        num_frames: int | None,
    ...
    def forward(self, batch: Batch, flows: Flows) -> BackboneOutput:
        ...
        # print(f"resize_shape={(h,w)}")
        # print(f"p_h={patch_h}, p_w={patch_w}")

        dino_features_fused = []
        shallow_dino_features = []
        for i, feature in enumerate(features):
            feature = feature.reshape(b, f, patch_h, patch_w, self.embed_dim)
            shallow_dino_features.append(feature) 
            dino_features_fused.append(feature.reshape(b*f, patch_h, patch_w, self.embed_dim))
        ...
        shallow_dino_features = rearrange(shallow_dino_features, "(b f) c h w -> b f h w c", b=b, f=f)   

        tokens_fused = tokens
        global_tokens_fused = global_tokens
        camera_tokens_fused = camera_tokens
